...
public class ChatUserService {
	...
	public void unblockUser(Long userId, Long blockTeamId) {
		...
		BlockChatUserEntity blockChatUser = blockUserRepository.findByUserIdAndBlockUserId(userId, blockUserId)
			.orElseThrow(
	...
	public BlockedUsersResponse getBlockUsers(Long userId) {
		...
		// TODO dto 변환하기
		List<UserDetail> userDetails = userRepository.findAllById(
			blockedUsers.stream().map(BlockChatUserEntity::getBlockUserId).toList());
